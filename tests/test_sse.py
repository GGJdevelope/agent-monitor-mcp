...
async def test_event_generation_multiple_agents(service):
    ...
    """
    service.update_status(StatusEvent(
        agent_id="agent-A", run_id="r1", task_name="t1",
        status=AgentStatus.RUNNING, progress=0, message="A"
    ))
    service.update_status(StatusEvent(
        agent_id="agent-B", run_id="r1", task_name="t1",
        status=AgentStatus.RUNNING, progress=0, message="B"
    ))
